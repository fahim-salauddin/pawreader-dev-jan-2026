...
export default function PsychicImageResult({ imageUrl, text }: Props) {
  ...
  const [faceSafeZone, setFaceSafeZone] = useState({ x: 0, y: 0, width: 0, height: 0 });
  ...
  // In this new useEffect, we'll define the pet's face safe zone.
  useEffect(() => {
  ...
  // This function checks if the bubble is in the pet's face safe zone.
  const isBubbleInSafeZone = (x: number, y: number, width: number, height: number) => {
    return (
      x < faceSafeZone.x + faceSafeZone.width &&
      x + width > faceSafeZone.x &&
      y < faceSafeZone.y + faceSafeZone.height &&
      y + height > faceSafeZone.y
    );
  };
  ...
  useEffect(() => {
    ...
    const scaledWidth = Math.max(isMobile ? 200 : 280, rect.width * (isMobile ? 0.8 : 0.5));
    setBubbleWidth(scaledWidth);
    
    let initialX = rect.width * 0.55;
    let initialY = rect.height * 0.65;

    // This will make sure the bubble doesn't start in the pet's face safe zone.
    if (isBubbleInSafeZone(initialX, initialY, scaledWidth, 100)) {
      initialX = 10;
      initialY = 10;
    }
  ...
  useEffect(() => {
    const handleMove = (e: any) => onMove(e);
    ...
    <div style={{ textAlign: "center" }}>
      ...
      {isMobile && (
        <div style={{
          ...
          If you are accessing Paw Reader in Mobile, rotate your phone horizontal for placing your paw paw's thoughts correctly :)
        </div>
      )}
      ...
      {showIosModal && (
        ...
        >
          <img src={iosImageUrl} alt="Generated pet psychic" style={{ maxWidth: '90%', maxHeight: '60%', borderRadius: 12 }} />
          <p style={{ color: 'white', marginTop: 20, fontSize: 18, textAlign: 'center', padding: '0 20px' }}>
            Tap and hold the image, then choose 'Add to Photos' to save.
          </p>
          <button 
            onClick={() => setShowIosModal(false)}
            style={{
